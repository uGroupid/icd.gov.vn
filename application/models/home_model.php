...
class Home_model extends CI_Model{
  ...
  function SearchDataDocument($string){
    $this->db->select('*');
		$this->db->like('title_vi',$string);
		$this->db->like('number_symbol',$string);
		$this->db->like('title_en',$string);
		$this->db->like('date_issued',$string);
  ...
  function select_box_document(){
    $this->db->select('*');
			$this->db->where('subcate',32);
			$this->db->from('tbl_danhmuc');
				$query = $this->db->get();
				return $query->result_array();
    
  }
  function tbl_getdocument($sub_menu){
			$this->db->select('*');
			$this->db->where('active',1);
			$this->db->limit(20);
			$this->db->order_by("id", "desc"); 
  ...
  function tbl_get_id_document($id){
    $this->db->select('*');
			$this->db->where('active',1);
			$this->db->where('id',$id);
			$this->db->limit(20);
  ...
  function tbl_get_id_documents($id,$ids){
    $this->db->select('*');
			$this->db->where('active',1);
			$this->db->where('sub_menu',$id);
	...
	function getdocument_submenu($sub_menu){
			$this->db->select('*');
			$this->db->where('id_danhmuc',$sub_menu);
			$this->db->from('tbl_danhmuc');
				$query = $this->db->get();
				return $query->result_array();
	}
	...
	function tbl_get_videohot($sub_menu){
			$this->db->select('*');
			$this->db->where('tin_hot_category =',1);
			$this->db->where('active',1);
			$this->db->where('id_menu',$sub_menu);
